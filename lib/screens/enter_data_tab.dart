...
class _EnterDataTabState extends State<EnterDataTab> {
  ...
  void _saveDetails() async {
    final userDetails = UserDetails(
      ...
      dependents: _dependentsController.text.isEmpty ? 'None' : _dependentsController.text,
    ...
    return SingleChildScrollView(
      child: Padding(
        ...
        child: Form(
          ...
          child: Column(
            children: [
              const Text('Enter Medical Details', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
              TextFormField(controller: _idNumberController, decoration: const InputDecoration(labelText: 'ID Number')),
              ...
              DropdownButtonFormField<BloodType>(
                ...
                items: _bloodTypes.map((type) => DropdownMenuItem(value: type, child: Text(type.toShortString()))).toList(),
                onChanged: (val) => setState(() => _selectedBloodType = val),
                decoration: const InputDecoration(labelText: 'Blood Type'),
              ),
              TextFormField(controller: _weightController, decoration: const InputDecoration(labelText: 'Weight (kg)'), keyboardType: const TextInputType.numberWithOptions(decimal: true)),
              TextFormField(controller: _conditionsController, decoration: const InputDecoration(labelText: 'Conditions')),
              TextFormField(controller: _allergiesController, decoration: const InputDecoration(labelText: 'Allergies')),
              TextFormField(controller: _currentMedicationsController, decoration: const InputDecoration(labelText: 'Current Medications')),
              TextFormField(controller: _pastMedicationsController, decoration: const InputDecoration(labelText: 'Past Medications')),
              TextFormField(controller: _pastSurgeriesController, decoration: const InputDecoration(labelText: 'Past Surgeries')),
              TextFormField(controller: _emergencyContactController, decoration: const InputDecoration(labelText: 'Emergency Contact'), keyboardType: TextInputType.phone, inputFormatters: [FilteringTextInputFormatter.digitsOnly]),
              TextFormField(controller: _phoneNumberController, decoration: const InputDecoration(labelText: 'Phone Number'), keyboardType: TextInputType.phone, inputFormatters: [FilteringTextInputFormatter.digitsOnly]),
              TextFormField(controller: _dependentsController, decoration: const InputDecoration(labelText: 'Dependents (comma-separated or "None")')),
              DropdownButtonFormField<GuardianType>(
                value: _selectedGuardianType,
                items: _guardianOptions.map((g) => DropdownMenuItem(value: g, child: Text(g.name))).toList(),
              ...
              TextFormField(controller: _doctorNotesController, decoration: const InputDecoration(labelText: 'Doctor Notes'), maxLines: 3),
              TextFormField(controller: _lastSyncController, decoration: const InputDecoration(labelText: 'Last Sync Timestamp')),
